...
export async function buildStaticData({ root = process.cwd(), fetchImpl = fetch, now = new Date(), log = console } = {}) {
  ...
  const completedFriday = completedWeekEnd(taipei);
...
function completedWeekEnd(taipei) {
  const date = new Date(`${taipei.date}T00:00:00Z`);
  const weekday = date.getUTCDay();
  const isClosedFriday = weekday === 5 && taipei.time >= "14:00:00";
  const offset = isClosedFriday ? 0 : (weekday - 5 + 7) % 7 || 7;
  return addDays(taipei.date, -offset);
}
