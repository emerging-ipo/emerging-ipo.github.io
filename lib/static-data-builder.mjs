...
export async function fetchJson(fetchImpl, url, init = {}, { attempts = 3, delayMs = 1_200 } = {}) {
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      if (!response.ok) {
        ...
        if (!shouldRetryFetchStatus(response.status) || attempt === attempts) throw error;
        lastError = error;
      } else {
...
export async function loadCompletedWeekReports(fetchImpl, weekEnd) {
  ...
  const reports = [];
  for (const date of dates) reports.push(await loadDailyReport(fetchImpl, date));
  return reports.filter(report => report.date && report.values.size);
}

async function loadDailyReport(fetchImpl, date) {
  try {
    return parseDailyAverageReport(await fetchJson(fetchImpl, TPEx_DAILY_REPORT_URL(date), {}, { attempts: 5 }));
  } catch (error) {
