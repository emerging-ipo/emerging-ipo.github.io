...
export function buildTrackerPayload({
  ...
  const items = (applicants || [])
    ...
    .filter(item => item.code && !isTerminatedApplication(`${item.status} ${item.note}`))
    ...
    .filter(item => !item.listingDate || item.listingDate > today)
...
export function isTerminatedApplication(note) {
  return /撤件|自撤|自行撤回|撤回(?:申請)?|撤銷.*(?:申請|契約)|退件|終止申請/.test(String(note || ""));
}
