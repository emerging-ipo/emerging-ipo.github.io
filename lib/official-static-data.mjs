...
export function buildMarketPayload({ generatedAt, companies, quotes, baselines = new Map() }) {
  ...
  const rows = mergeRecentRegistrations(companies || [], []).map(company => {
    ...
    const lastWeekAverage = numberValue(baseline?.average ?? baseline?.close ?? baseline?.value);
    ...
    const change = average !== null && lastWeekAverage !== null && lastWeekAverage !== 0
      ? round(average / lastWeekAverage - 1, 6) : null;
    ...
    return {
      ...
      priceNote: baseline?.date ? "" : "上週無有效加權均價",
...
function enrichApplicant(item, auction, offering, marketRow, baseline, today) {
  ...
  const lastWeekAverage = numberValue(marketRow?.lastWeekAverage ?? marketRow?.lastWeekClose ?? baseline?.average ?? baseline?.close ?? baseline?.value);
  const weeklyChange = currentAverage !== null && lastWeekAverage !== null && lastWeekAverage !== 0
    ? round(currentAverage / lastWeekAverage - 1, 6) : "";
  ...
  return {
    ...
    priceNote: cleanText(marketRow?.priceNote) || (baseline?.date ? "" : "上週無有效加權均價"),
    weeklyChange,
...
function radarRow(item, today) {
  ...
  return {
    ...
    lastWeekClose: displayPrice(item.lastWeekAverage),
