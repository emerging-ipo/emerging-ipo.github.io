...
export function buildMarketPayload({ generatedAt, companies, quotes, baselines = new Map() }) {
  ...
  const rows = mergeRecentRegistrations(companies || [], []).map(company => {
    ...
    const latest = volume > 0 ? numberValue(valueOf(quote, "LatestPrice", "成交")) : null;
    const average = volume > 0 ? numberValue(valueOf(quote, "Average", "日均價")) : null;
    const turnover = Math.round((average ?? latest ?? 0) * volume);
    const baseline = baselines.get(company.code) || null;
    const lastWeekAverage = numberValue(baseline?.average);
    const lastWeekClose = numberValue(baseline?.close ?? baseline?.value);
    const dailyChange = average !== null && previousAverage !== null ? round(average - previousAverage, 2) : null;
    const dailyChangePercent = average !== null && previousAverage !== null && previousAverage !== 0
      ? round(average / previousAverage - 1, 6) : null;
    const change = latest !== null && lastWeekClose !== null && lastWeekClose !== 0
      ? round(latest / lastWeekClose - 1, 6) : null;
    ...
    return {
      ...
      turnover,
      change,
      // Legacy aliases retained for existing tracker JSON consumers. These values are weighted averages, not closes.
      previousClose: previousAverage,
      previousCloseDate: quoteDate,
      dailyChange,
      dailyChangePercent,
      lastWeekAverage,
      lastWeekAverageDate: baseline?.date || "",
      lastWeekClose,
      ...
      priceSource: "公開行情",
      priceError: latest === null ? "無可用最後成交價" : "",
      priceNote: baseline?.date ? "" : "上週無有效最後成交價",
...
export function selectWeeklyBaselines(reports, lastWeekEnd) {
  ...
  for (const report of sorted) {
    for (const [code, value] of reportEntries(report)) {
      const average = numberValue(value?.average ?? value?.close ?? value?.value ?? value);
      if (/^\d{4}$/.test(code) && average !== null && average > 0 && !result.has(code)) {
        // `close` remains as a compatibility alias. The official report value is a weighted average transaction price.
        result.set(code, { average, close: average, date: report.date });
      }
    }
  }
  return result;
...
export function updateQuoteHistory(history, quotes, { maxSnapshots = 60 } = {}) {
  const quoteDate = (quotes || []).map(row => toIsoDate(valueOf(row, "Date", "日期"))).filter(Boolean).sort().at(-1) || "";
  const snapshots = new Map((history?.snapshots || [])
    .map(normaliseQuoteSnapshot)
    .filter(snapshot => snapshot.date && snapshot.rows.length)
    .map(snapshot => [snapshot.date, snapshot]));
  const rows = (quotes || [])
    .filter(row => toIsoDate(valueOf(row, "Date", "日期")) === quoteDate)
    .map(row => ({
      code: codeOf(row),
      latest: numberValue(valueOf(row, "LatestPrice", "成交")),
      volume: integerValue(valueOf(row, "TransactionVolume", "成交量"))
    }))
    .filter(row => /^\d{4}$/.test(row.code) && row.latest !== null && row.latest > 0 && row.volume > 0)
    .sort((a, b) => a.code.localeCompare(b.code));

  if (quoteDate && rows.length) snapshots.set(quoteDate, { date: quoteDate, rows });
  return {
    version: 1,
...
export function selectWeeklyLastTrades(snapshots, lastWeekEnd) {
  const weekStart = addDays(lastWeekEnd, -4);
  const result = new Map();
  const sorted = [...(snapshots || [])]
    .map(normaliseQuoteSnapshot)
    .filter(snapshot => snapshot.date >= weekStart && snapshot.date <= lastWeekEnd)
    .sort((a, b) => b.date.localeCompare(a.date));

  for (const snapshot of sorted) {
    for (const row of snapshot.rows) {
      if (/^\d{4}$/.test(row.code) && row.latest !== null && row.latest > 0 && row.volume > 0 && !result.has(row.code)) {
        result.set(row.code, { close: row.latest, date: snapshot.date });
      }
    }
  }
  return result;
...
export function completedWeekEndForQuoteDate(quoteDate) {
  const date = toIsoDate(quoteDate);
  if (!date) return "";
  const weekday = new Date(`${date}T00:00:00Z`).getUTCDay();
  const mondayOffset = weekday === 0 ? 6 : weekday - 1;
  const currentWeekMonday = addDays(date, -mondayOffset);
  return addDays(currentWeekMonday, -3);
}
...
function enrichApplicant(item, auction, offering, marketRow, baseline, today) {
  ...
  const currentAverage = numberValue(marketRow?.average);
  const lastWeekAverage = numberValue(marketRow?.lastWeekAverage ?? baseline?.average);
  ...
  const weeklyChange = currentPrice !== null && lastWeekClose !== null && lastWeekClose !== 0
    ? round(currentPrice / lastWeekClose - 1, 6) : "";
  ...
  return {
    ...
    priceNote: cleanText(marketRow?.priceNote) || (baseline?.date ? "" : "上週無有效最後成交價"),
    weeklyChange,
...
function radarRow(item, today) {
  ...
  return {
    ...
    currentAverage: displayPrice(item.currentAverage),
    lastWeekAverage: displayPrice(item.lastWeekAverage),
...
function normaliseQuoteSnapshot(snapshot) {
  return {
    date: toIsoDate(snapshot?.date),
    rows: (snapshot?.rows || []).map(row => ({
      code: codeOf(row),
      latest: numberValue(valueOf(row, "latest", "LatestPrice", "成交")),
      volume: integerValue(valueOf(row, "volume", "TransactionVolume", "成交量"))
