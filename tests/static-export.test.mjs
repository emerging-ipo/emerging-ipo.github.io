...
test("GitHub Pages workflow deploys the static output", async () => {
  ...
  assert.match(workflow, /cron:\s*["']10 16 \* \* 1-5["']/);
  assert.match(workflow, /cron:\s*["']40 17 \* \* 1-5["']/);
  assert.match(workflow, /cron:\s*["']10 18 \* \* 1-5["']/);
  assert.match(workflow, /cron:\s*["']0 10 \* \* 6["']/);
  assert.equal((workflow.match(/timezone:\s*["']Asia\/Taipei["']/g) || []).length, 4);
  assert.doesNotMatch(workflow, /10 7 \* \* 1-5/);
  assert.doesNotMatch(workflow, /10 8 \* \* 1-5|40 9 \* \* 1-5|10 10 \* \* 1-5|0 2 \* \* 6/);
