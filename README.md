...
The production repository is `emerging-ipo/emerging-ipo.github.io`. GitHub Actions builds the static `out` directory and publishes it to GitHub Pages.

The scheduled build requests updates at 16:10, 17:40 and 18:10 Taiwan time on weekdays (`10 8`, `40 9` and `10 10` UTC), plus a final Saturday 10:00 reconciliation (`0 2` UTC). UTC cron values are used because this repository's August 31 runs showed the timezone-aware entries being dispatched about eight hours late. GitHub can also delay or drop scheduled events; these are requested times, not a guarantee that the site has updated. A successful push or manual run does not prove that the next scheduled event will arrive. The workflow records its trigger and actual start time, and the builder logs the official quote date separately from the generation time.

You can run the same update manually from GitHub: **Actions** → **Deploy GitHub Pages** → **Run workflow**. For scheduling independent of GitHub's cron dispatcher, an external cloud scheduler must call the existing `workflow_dispatch` endpoint: `POST /repos/emerging-ipo/emerging-ipo.github.io/actions/workflows/pages.yml/dispatches`, with body `{"ref":"main"}` and a repository-scoped token with Actions write permission. Keep that token only in the scheduler's secret storage, never in this public repository. No external scheduler has been configured by this repository alone.

Transient official-source failures (including HTTP 307, 429, server errors and interrupted response bodies) receive up to five attempts with exponential backoff. If the server provides `Retry-After`, it is honored. Exhausted retries still fail the build; they never substitute empty data or present an old snapshot as a fresh successful update.
