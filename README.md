...
The scheduled build requests updates at 16:10, 17:40 and 18:10 on weekdays, plus a final Saturday 10:00 reconciliation. Each cron entry explicitly uses `timezone: "Asia/Taipei"`. GitHub can delay or drop scheduled events; these are requested times, not a guarantee that the site has updated. A successful push or manual run does not prove that the next scheduled event will arrive. The workflow records its trigger and actual start time, and the builder logs the official quote date separately from the generation time.
